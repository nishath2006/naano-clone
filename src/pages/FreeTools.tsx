...
const COPY = {
  en: EN,
  fr: { ...EN, badge: 'Outils gratuits par Naano', open: "Ouvrir l'outil" },
}
      ...
      <section className="relative overflow-hidden pt-28 pb-12 sm:pt-36 sm:pb-16">
        ...
        <div className="relative mx-auto max-w-[900px] px-4 sm:px-6 text-center">
          <span className="inline-flex min-h-10 items-center gap-2 rounded-full border border-[#E4E1DC] bg-white px-4 py-2 text-[13px] font-medium text-[#55575E] shadow-[0_1px_2px_rgba(23,24,28,0.04)]">
            <SparklesIcon size={14} className="text-[#1652F0]" />
            {c.badge}
          </span>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 sm:gap-6">
            {cards.map((card) => (
              ...
              >
                ...
                <p className="mt-1.5 text-[15px] font-medium text-[#1652F0]">{card.tagline}</p>
            ...
            <div className="flex flex-col justify-center rounded-2xl border border-dashed border-[#E4E1DC] bg-[#FAFAF9] p-7">
              ...
              <p className="mt-3 text-[15px] leading-relaxed text-[#55575E]">
                ...
                <Link
                  to="/blog"
                  className="font-semibold text-[#1652F0] underline underline-offset-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1652F0] rounded-sm"
                >
                  {c.coming.blog}
                </Link>{' '}
                {c.coming.after}
        ...
        <ul className="mt-6 divide-y divide-[#ECEAE6]">
          {KEEP_READING.map((l) => (
            <li key={l.href}>
              <Link
                to={l.href}
                className="group flex min-h-11 items-center justify-between gap-4 py-3.5 text-[16px] font-medium text-[#17181C] transition-colors hover:text-[#1652F0] motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1652F0] rounded-sm"
              >
                {l.label}
                <ArrowRightIcon
                  size={16}
